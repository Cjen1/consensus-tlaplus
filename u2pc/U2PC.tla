---- MODULE U2PC ----
EXTENDS FiniteSets, Integers, Apalache
...
ASSUME \A k1, k2 \in DOMAIN Shards: k1 /= k2 => Shards[k1] \cup Shards[k2] = {}
ASSUME "Init" \notin DOMAIN Txns
...
Init ==
  /\ Replicas = [r \in RIDs |-> [locked |-> FALSE, version |-> "Init", logged |-> "Init"]]
...
CoordinatorLock(t) ==
  ...
  /\ M_lock' = M_lock \cup {[src |-> t, txn |-> Txns[t], state |-> Coordinator_txn_state[t]]}
...
ReplicaLock(r) ==
  ...
  /\ \E m \in M_lock:
     ...
     /\ IF Replicas[r].version = m.state[KeyLookup[r]]
        THEN 
...
CoordinatorCommit(t) ==
  /\ Coordinator_txn_state[t] = "Decide"
  /\ \A k \in Txns
